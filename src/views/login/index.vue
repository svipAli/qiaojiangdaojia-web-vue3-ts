        ...
        <div class="login-container">
          ...
          <div class="login-ID">
            ...
            <lay-tab type="brief" v-model="method">
              <lay-tab-item title="账户密码登录" id="1">
                <div style="height: 250px" @keyup.enter="loginSubmit">
                  ...
                  <lay-form-item :label-width="0">
                    <div style="width: 264px; display: inline-block">
                      <lay-input :allow-clear="true" prefix-icon="layui-icon-vercode" placeholder="验证码"
                                 v-model="loginForm.img_code"></lay-input>
                    </div>
                    <div class="login-captach" @click="toRefreshImg">
                      <img style="width: 100%" :src="code_img" alt="获取验证码"/>
                    </div>
                  </lay-form-item>
...
export default defineComponent({
  setup() {
        ...
        try {
          ...
          if (access_token.length>0 && token_type.length>0) {
            layer.msg("登录成功", {icon: 1}, async () => {
              userStore.token = `${token_type} ${access_token}`
              router.push('/')
              return
            })
          }
    ...
    const toRefreshImg = async () => {
      let data = await verificationImg()
      code_img.value = data.code_img
